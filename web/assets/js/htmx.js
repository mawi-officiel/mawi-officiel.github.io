...
class CustomHTMX {
    ...
    executeScripts() {
        ...
        scripts.forEach(script => {
            ...
            if (script.getAttribute('src')) {
                ...
                // Copy other attributes (except src, type, and HTMX markers)
                Array.from(script.attributes).forEach(attr => {
                    const name = attr.name;
                    if (name !== 'src' && name !== 'type' && name !== 'data-htmx-executed' && name !== 'data-htmx-template') {
                        newScript.setAttribute(name, attr.value);
                    }
                });
                
                // Mark the new script as executed
                newScript.setAttribute('data-htmx-executed', 'true');
                ...
                try {
                    ...
                    // Copy other attributes (except HTMX markers)
                    Array.from(script.attributes).forEach(attr => {
                        const name = attr.name;
                        if (name !== 'type' && name !== 'data-htmx-executed' && name !== 'data-htmx-template') {
                            newScript.setAttribute(name, attr.value);
                        }
                    });
    ...
    extractAndApplyHead(templateHTML) {
        try {
            ...
            // Return the body content (excluding head)
            if (doc.body) {
            ...
            // Fallback: strip head via regex if body is not present
    ...
    updateDocumentHead(headEl) {
        ...
        // Resource hints: safely handle preload links to avoid console warnings for late-loaded templates
        try {
            // Remove existing JS preloads that target ads-manager.js (can trigger warning if consumed late)
            Array.from(document.head.querySelectorAll('link[rel="preload"], link[rel="modulepreload"]')).forEach(link => {
                const href = link.getAttribute('href') || '';
                if (/assets\/js\/services\/ads-manager\.js(\?.*)?$/i.test(href)) {
                    link.remove();
                }
            });
            // Add non-JS preloads from template head and ensure proper `as` values
            headEl.querySelectorAll('link[rel="preload"], link[rel="modulepreload"]').forEach(link => {
                const href = link.getAttribute('href') || '';
                const rel = (link.getAttribute('rel') || '').toLowerCase();
                const asAttr = (link.getAttribute('as') || '').toLowerCase();
                const isJs = /\.js(\?.*)?$/i.test(href);
                const isCss = /\.css(\?.*)?$/i.test(href);
                const isFont = /(\.woff2?|\.ttf|\.otf)(\?.*)?$/i.test(href);
                // Skip JS preloads to prevent "preloaded but not used" warnings when HTMX injects scripts after window load
                if (isJs) return;
                // Patch missing or incorrect `as` attribute
                if (!asAttr) {
                    if (isCss) link.setAttribute('as', 'style');
                    else if (isFont) link.setAttribute('as', 'font');
                }
                let exists = null;
                try {
                    exists = document.head.querySelector(`link[rel="${CSS.escape(rel)}"][href="${CSS.escape(href)}"]`);
                } catch (_) {
                    exists = Array.from(document.head.querySelectorAll('link[rel]')).find(el => (el.getAttribute('rel') || '').toLowerCase() === rel && (el.getAttribute('href') || '') === href);
                }
                if (!exists) {
            ...
            console.warn('HTMX: preload handling failed:', e);
        }
        ...
        headEl.querySelectorAll('script').forEach(script => {
            ...
            if (srcAttr) {
                ...
                try {
                    ...
                    exists = Array.from(document.head.querySelectorAll('script[src]')).find(el => (el.getAttribute('src') || '') === srcAttr);
                }
                if (!exists) {
                    const newScript = document.createElement('script');
                    newScript.src = srcAttr;
                    newScript.type = script.getAttribute('type') || 'text/javascript';
                    // For classic scripts in head, prefer defer to avoid blocking
                    if (newScript.type !== 'module' && !script.hasAttribute('async')) {
                        newScript.setAttribute('defer', '');
                    }
                    // Copy other attributes except src/type
                    Array.from(script.attributes).forEach(attr => {
                        const name = attr.name;
                    ...
                    newScript.setAttribute('data-htmx-head', 'true');
    ...
    async loadTemplate(element, templateName) {
        ...
        try {
            ...
            // Replace the htmx element with the template body content
